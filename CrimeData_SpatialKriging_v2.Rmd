---
title: "Rasterized Crime Data Spatial Kriging"
date: "April 20, 2015"
...
Variogram models can be fitted to the sample variogram using iterative reweighted least squares (IRLS) estimation, or can be fitted directly to the sample data using Resitricted (residual) Maximum Likelihood (REML) estimation. Appendix A1 in *gstat* manual gives details on the calculation of sample variograms and model fitting. Non-linear least squares fitting is only guaranteed to work when good initial values are provided. Therefore, and more in general, visual examination of model fit is recommended.

```{r vgm-cloud, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center', cache=TRUE, eval=TRUE}
# variogram cloud
...
Weighted Least Square (WLS) fitting:
...
REML fitting (time demanding):

```{r vgm-mean_ML, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center', cache=TRUE, eval=TRUE}
# gaussian model does not converge in limited iterations

# REML is time demanding and very senstive to the initial values
vgm.sph_ml = vgm(psill=5e6,model="Sph",range=30000,nugget=5e5)
vgm.exp_ml = vgm(psill=5e6,model="Exp",range=30000,nugget=5e5)
# vgm.gau_ml = vgm(psill=4e5,model="Gau",range=40000,nugget=5e4)

ptm <- proc.time()
INC.sphfit_ML = fit.variogram.reml(INC_CNT~1, data = sub.df, model=vgm.sph_ml, set=list(iter=50))
INC.expfit_ML = fit.variogram.reml(INC_CNT~1, data = sub.df, model=vgm.exp_ml, set=list(iter=50))
# INC.gaufit_ML = fit.variogram.reml(INC_CNT~1, data = sub.df, model=vgm.gau_ml, set=list(iter=30))
proc.time()-ptm

vL1 = variogramLine(INC.sphfit_ML, maxdist = max(INC.vgm$dist))
vL2 = variogramLine(INC.expfit_ML, maxdist = max(INC.vgm$dist))
# vL3 = variogramLine(INC.gaufit_ML, maxdist = max(INC.vgm$dist))

# ymx <-  max(c(vL1$gamma,vL2$gamma,vL3$gamma))
ymx <-  max(c(vL1$gamma,vL2$gamma))

# sphere variogram model
vL1 = variogramLine(INC.sphfit_ML, maxdist = max(INC.vgm$dist))
plot(x = vL1$dist, y = vL1$gamma, col = "red", type = "l", lty = 1, lwd=1.25) 
# exponetial variogram model
vL2 = variogramLine(INC.expfit_ML, maxdist = max(INC.vgm$dist))
lines(x = vL2$dist, y = vL2$gamma, col = "green", lty = 1, lwd=1.25)
title(main=list("REML fitting", cex=1))
```
