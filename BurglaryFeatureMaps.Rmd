...
```{r spatial-feature-maps, echo=FALSE, message=FALSE, warning=FALSE,fig.align='center',fig.width=10, fig.height=4.5, cache=TRUE}
...
RegGrdRaster.Dist2CPDstation <- rasterize(RegGrd[,c("X_COORD","Y_COORD")], r, RegGrd$Dist2CPDstation, fun=sum)
RegGrdRaster.Dist2School <- rasterize(RegGrd[,c("X_COORD","Y_COORD")], r, RegGrd$Dist2School, fun=sum)
RegGrdRaster.Dist2Park <- rasterize(RegGrd[,c("X_COORD","Y_COORD")], r, RegGrd$Dist2Park, fun=sum)
RegGrdRaster.Dist2Hospital <- rasterize(RegGrd[,c("X_COORD","Y_COORD")], r, RegGrd$Dist2Hospital, fun=sum)
RegGrdRaster.Dist2Library <- rasterize(RegGrd[,c("X_COORD","Y_COORD")], r, RegGrd$Dist2Library, fun=sum)
RegGrdRaster.Dist2BusStop <- rasterize(RegGrd[,c("X_COORD","Y_COORD")], r, RegGrd$Dist2BusStop, fun=sum)
RegGrdRaster.Dist2CTAroute<- rasterize(RegGrd[,c("X_COORD","Y_COORD")], r, RegGrd$Dist2CTAroute, fun=sum)
RegGrdRaster.Dist2CTArail <- rasterize(RegGrd[,c("X_COORD","Y_COORD")], r, RegGrd$Dist2CTArail, fun=sum)
RegGrdRaster.BldgDen <- rasterize(RegGrd[,c("X_COORD","Y_COORD")], r, RegGrd$BldgDen, fun=sum)

jet.colors <- colorRampPalette(c("#00007F", "blue", "#007FFF", "cyan", "#7FFF7F", "yellow", "#FF7F00", "red", "#7F0000"))
par.default <- par()
par(mfrow=c(1,2))
plot(RegGrdRaster.StrDen,col=jet.colors(256), main="Nearby Street Density",cex.main=0.75,
     panel.first=grid(grd.full@grid@cells.dim[2], grd.full@grid@cells.dim[1],col="lightgray", lty="dotted"))
plot(RegGrdRaster.Dist2Street,col=jet.colors(256), main="Distance To Major Street",cex.main=0.75,
     panel.first=grid(grd.full@grid@cells.dim[2], grd.full@grid@cells.dim[1],col="lightgray", lty="dotted"))
plot(RegGrdRaster.BldgDen,col=jet.colors(256), main="Nearby Building Density",cex.main=0.75,
     panel.first=grid(grd.full@grid@cells.dim[2], grd.full@grid@cells.dim[1],col="lightgray", lty="dotted"))
plot(RegGrdRaster.Dist2CPDstation,col=jet.colors(256), main="Distance to Police Station",cex.main=0.75,
     panel.first=grid(grd.full@grid@cells.dim[2], grd.full@grid@cells.dim[1],col="lightgray", lty="dotted"))
plot(RegGrdRaster.Dist2School,col=jet.colors(256), main="Distance to School",cex.main=0.75,
     panel.first=grid(grd.full@grid@cells.dim[2], grd.full@grid@cells.dim[1],col="lightgray", lty="dotted"))
plot(RegGrdRaster.Dist2Park,col=jet.colors(256), main="Distance to Park",cex.main=0.75,
     panel.first=grid(grd.full@grid@cells.dim[2], grd.full@grid@cells.dim[1],col="lightgray", lty="dotted"))
plot(RegGrdRaster.Dist2Hospital,col=jet.colors(256), main="Distance to Hospital",cex.main=0.75,
     panel.first=grid(grd.full@grid@cells.dim[2], grd.full@grid@cells.dim[1],col="lightgray", lty="dotted"))
plot(RegGrdRaster.Dist2Library,col=jet.colors(256), main="Distance to Library",cex.main=0.75,
     panel.first=grid(grd.full@grid@cells.dim[2], grd.full@grid@cells.dim[1],col="lightgray", lty="dotted"))
plot(RegGrdRaster.Dist2BusStop,col=jet.colors(256), main="Distance to Bus Stop",cex.main=0.75,
     panel.first=grid(grd.full@grid@cells.dim[2], grd.full@grid@cells.dim[1],col="lightgray", lty="dotted"))
plot(RegGrdRaster.Dist2CTAroute,col=jet.colors(256), main="Distance to Bus route",cex.main=0.75,
     panel.first=grid(grd.full@grid@cells.dim[2], grd.full@grid@cells.dim[1],col="lightgray", lty="dotted"))
plot(RegGrdRaster.Dist2CTArail,col=jet.colors(256), main="Distance to CTA Train Rail",cex.main=0.75,
     panel.first=grid(grd.full@grid@cells.dim[2], grd.full@grid@cells.dim[1],col="lightgray", lty="dotted"))
```
...
```{r visualize-kernels, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center',fig.width=10, fig.height=4.5, cache=TRUE}
# visualize the kernels
library(plot3D)
par(mfrow=c(1,2))
mesh.xy <- mesh(x=kernel.Xgrd,y=kernel.Ygrd)
kernel.xy <- with(mesh.xy, 1/(2*pi*sigma^2)*exp(-(x^2+y^2)/(2*sigma^2)))
surf3D(x=mesh.xy$x,y=mesh.xy$y,z=kernel.xy, phi=30, theta=60, facets=FALSE,main="Spatial Gaussian Kernel",
       cex.main=0.75,colkey=list(width=0.5))
plot(kernel.Tgrd, kernel.t,pch=16,type="b",xlab=expression(t),ylab=expression(e^(-lambda %.% t)),
     main="Temporal Exponential Filter",cex.main=0.75)
```
...
for (i in 1:Ndays.train){
  ...
  FeatureVec.train[((i-1)*Ngrids+1):(i*Ngrids),VarName.crime] <- KS$KernSm.df_inPoly$KS_VAL
...
for (i in 1:Ndays.test){
  ...
  FeatureVec.test[((i-1)*Ngrids+1):(i*Ngrids),VarName.crime] <- KS$KernSm.df_inPoly$KS_VAL
...
```{r visualize-kernelSmooth, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center',fig.width=10, fig.height=4.5, cache=TRUE}
# visualize an example (the last one)
par(mfrow=c(1,2))
KernSm.raster_inCity <- rasterize(KS$KernSm.df_inPoly[,c("X_COORD","Y_COORD")], r, KS$KernSm.df_inPoly$KS_VAL, fun=sum)
plot(KernSm.raster_inCity,col=jet.colors(256), main="Rasterized Kernel Smoothed Counts of 14 Preceding Days of Day t and Neighboring Locations",cex.main=0.75,panel.first=grid(grd.full@grid@cells.dim[2], grd.full@grid@cells.dim[1],col="lightgray", lty="dotted"))
plot(BurglaryRaster,col=jet.colors(256), main="Rasterized Actual Incident Counts of the Day t",
     cex.main=0.75,panel.first=grid(grd.full@grid@cells.dim[2], grd.full@grid@cells.dim[1],col="lightgray", lty="dotted"))
``` 

Using one year training data (data of year 2014 here), we do univariate aanalysis for each feature to see its separability for the classification problem. 
1. The temporal features are not location-depended, so we use box plots to see the city wide incident counts varying by time.
2. The spatial and near-repeat effect features are presented as histograms for each class. Also their corresponding smoothed density curves are shown below the histograms.

```{r univariate-analysis, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center',fig.width=10, fig.height=4.5, cache=TRUE}
library(lattice)

BurglaryData.train2 <- subset(BurglaryData,DATEOCC>=startDate.train & DATEOCC<=endDate.train)
BurglaryData.daily <- aggregate(INC_CNT~DATEOCC+YEAR+MONTH+DOW+HOLIDAY,data=BurglaryData.train2, FUN=sum, na.rm=TRUE)
BurglaryData.daily <- BurglaryData.daily[order(BurglaryData.daily$DATEOCC),]
BurglaryData.daily$DOW <- factor(BurglaryData.daily$DOW, levels=c("Sun","Mon","Tue","Wed","Thu","Fri","Sat"))
BurglaryData.daily$HOLIDAY <- factor(BurglaryData.daily$HOLIDAY, levels=levels(BurglaryData$HOLIDAY))
BurglaryData.daily$MONTH <- factor(BurglaryData.daily$MONTH)

par(mfrow=c(1,3))
boxplot(INC_CNT~MONTH,BurglaryData.daily,xaxt="n",ylab="Burglary Count",
...
boxplot(INC_CNT~DOW,BurglaryData.daily,ylab="Burglary Count",
        main="City-wide daily burglary incidents by day of week",cex.main=0.8)
boxplot(INC_CNT~HOLIDAY,BurglaryData.daily,ylab="Burglary Count",
        main="City-wide daily burglary incidents by holiday",cex.main=0.8)

TrainData <- cbind(subset(FeatureVec.train,select=-c(DAY,YEAR)),Label3c.train,Label2c.train)

for(j in 1:length(VarName.space)){
  f <- histogram(as.formula(paste0("~",VarName.space[j],"|Label3c.train")), data=TrainData,type="density",breaks=50)
  print(f)
}

f <- histogram(as.formula(paste0("~",VarName.crime,"|Label3c.train")), data=TrainData,type="density",breaks=50)
print(f)


for(j in 1:length(VarName.space)){
  f <- densityplot(as.formula(paste0("~",VarName.space[j])), data=TrainData, groups=Label3c.train,
                   par.settings = list(superpose.symbol=list(pch=16,cex=1,col=c("blue","green","red")),
                                       superpose.line = list(col=c("blue","green","red"))),                                                                                                                                                   
                   plot.points=FALSE, ref=TRUE, 
                   auto.key=list(col=c("blue","green","red"),lwd=2,space="top",columns=3))

  if (j %% 2 ==1){
    print(f, position=c(0, 0, 0.4, 1), more=TRUE)
    }
  else{
    print(f, position=c(0.6, 0, 1, 1))
    }
}


f <- densityplot(as.formula(paste0("~",VarName.crime)), data=TrainData, groups=Label3c.train,
                 par.settings = list(superpose.symbol=list(pch=16,cex=1,col=c("blue","green","red")),
                                     superpose.line = list(col=c("blue","green","red"))),                                                                                                                                                   
                 plot.points=FALSE, ref=TRUE, 
                 auto.key=list(col=c("blue","green","red"),lwd=2,space="top",columns=3))
