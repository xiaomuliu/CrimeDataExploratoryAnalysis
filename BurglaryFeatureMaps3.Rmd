...
# VERSION 3
# Further Trim Building Shapefile Data
# DIFFERENTIATE PROPERTY (RESIDENTIAL GARAGE, RESIDENTIAL/COMMERCIAL BUILDING) IN CALCULATING NEARBY BUILDING DENSITY
# ADD BUILDING TYPE FEATURES
```
...
RegGrdRaster.GarageDen <- rasterize(RegGrd[,c("X_COORD","Y_COORD")], r, RegGrd$GarageDen, fun=sum)
RegGrdRaster.AvgStories <- rasterize(RegGrd[,c("X_COORD","Y_COORD")], r, RegGrd$AvgStories, fun=sum)
RegGrdRaster.AvgUnits <- rasterize(RegGrd[,c("X_COORD","Y_COORD")], r, RegGrd$AvgUnits, fun=sum)
...
plot(RegGrdRaster.Dist2CTArail,col=jet.colors(256), main="Distance to CTA Train Rail",cex.main=0.75,
     panel.first=grid(grd.full@grid@cells.dim[2], grd.full@grid@cells.dim[1],col="lightgray", lty="dotted"))
plot(RegGrdRaster.BldgDen,col=jet.colors(256), main="Number of Nearby Buildings",cex.main=0.75,
     panel.first=grid(grd.full@grid@cells.dim[2], grd.full@grid@cells.dim[1],col="lightgray", lty="dotted"))
plot(RegGrdRaster.GarageDen,col=jet.colors(256), main="Number of Nearby Residential Garages",cex.main=0.75,
     panel.first=grid(grd.full@grid@cells.dim[2], grd.full@grid@cells.dim[1],col="lightgray", lty="dotted"))
plot(RegGrdRaster.AvgStories,col=jet.colors(256), main="Average Number of Nearby Building Stories",cex.main=0.75,
     panel.first=grid(grd.full@grid@cells.dim[2], grd.full@grid@cells.dim[1],col="lightgray", lty="dotted"))
plot(RegGrdRaster.AvgUnits,col=jet.colors(256), main="Average Number of Nearby Building Units",cex.main=0.75,
     panel.first=grid(grd.full@grid@cells.dim[2], grd.full@grid@cells.dim[1],col="lightgray", lty="dotted"))
```
...
VarName.space <- c("StrDen","Dist2Street","Dist2CPDstation","Dist2School","Dist2Park","Dist2Hospital",
                   "Dist2Library","Dist2BusStop","Dist2CTAroute","Dist2CTArail","BldgDen","GarageDen","AvgStories","AvgUnits")
...
f <- densityplot(as.formula(paste0("~",VarName.crime)), data=TrainData, groups=Label3c.train,
                 ...
                 plot.points=FALSE, ref=TRUE, 
                 auto.key=list(col=c("blue","green","red"),lwd=2,space="top",columns=3))
if (length(VarName.space) %% 2==1){
  print(f,position=c(0.6,0,1,1))
}else{
  print(f, position=c(0, 0, 0.4, 1))
}
