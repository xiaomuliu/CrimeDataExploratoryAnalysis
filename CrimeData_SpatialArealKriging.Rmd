---
title: "Area-based Violent Crime Spatial Kriging"
date: "April 23, 2015"
...
In this part, cross-sectional data was aggregated by policing beat and then interpolated into a finer grid through space-only Kriging.

First of all, we need to add up points over beats. In the following analysis, we will be using all available data from year 2008 to year 2014.
...
library(lattice)
# variogram cloud
vgmCld_a.beat = variogram(INC_CNT~1,locations=coordinates(beat_all.spdf),data=beat_all.spdf,cloud=TRUE)
...
vgm.beat = variogram(INC_CNT~1, data = beat_all.spdf)
vgmSph.beat= vgm(model="Sph",range=35000, nugget=2.5e5)
vgmExp.beat= vgm(model="Exp",range=35000, nugget=1e5)
vgmGau.beat= vgm(model="Gau",range=35000, nugget=1e5)
...
xyplot(gamma ~ dist, vgm.beat,
       ...
       ylab = list(label="semivariance",cex=1), xlab = list(label="distance",cex=1),ylim=c(0,2e6),
...
vgmR.beat = variogram(INC_CNT~coordinates(beat_all.spdf), data = beat_all.spdf)
# plot(vgmR.beat)
vgm.sph= vgm(model="Sph",range=35000, nugget=1e5)
...
dirvgm.beat = variogram(INC_CNT~1, data = beat_all.spdf, cutoff = 1e5, alpha=c(0,45,90,135))
plot(dirvgm.beat,as.table=TRUE)
...
for (j in 1:12){  
  ...
  INCm.vgm = variogram(INC_CNT~1,data=beat_month.spdf)
  par(mar=c(4,4,3,1.5))
  plot(INCm.vgm$dist,INCm.vgm$gamma,pch=1,col="blue", main=as.character(j),
       ylab="semivariance", xlab="distance",ylim=c(0,18000))
}
...
for (j in unique(CrimeData.beat_year$YEAR)){  
  ...
  INCy.vgm = variogram(INC_CNT~1,data=beat_year.spdf)
  par(mar=c(4,4,3,1.5))
  plot(INCy.vgm$dist,INCy.vgm$gamma,pch=1,col="blue", main=as.character(j),
       ylab="semivariance", xlab="distance",ylim=c(0,60000))
}
title(main=list("Variogram for each year", cex=1.25),outer=TRUE)
```

Now let's move forward to Kriging. We fit the sample variogram by a spherical model. Based on this variogram model, we use Gaussian process to interpolate values on a finer (300 $\times$ 300) grid. To relieve the computational burden, local kriging (here 100 nearest observations was used) was applied instend of using all the observations for a single point kriging prediction.
...
nobs <- 100 
ptm <- proc.time()
INC.kriged = krige(INC_CNT~1, locations = beat_all.spdf, newdata = interp.grd, nmax = nobs, model = sphfit.beat)
