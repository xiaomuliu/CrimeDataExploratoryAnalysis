...
In this part, cross-sectional data was pooled and used for space-only Gaussian process (a.k.a Kriging in geo-spatial field) prediction.

First of all, we need to add up points over a coarse grids. The incident data was rasterized through binning into a 50 $\times$ 50 grid (the boundaries were defined by the range of x-coordinate and y-coordinate from all available crime locations plus a margin of 1000 unit on each side). The resolution of a finer grid which will be interpolated on is 300 $\times$ 300
...
# count data (Poisson distribution): use log transform 
eps = 1e-6

# omnidirectional variogram
INC.vgm = variogram(log(INC_CNT+eps)~1, cutoff = 1e5, data = sub.df)
vgm.sph= vgm(model="Sph",range=40000,nugget=10)
vgm.exp= vgm(model="Exp",range=40000,nugget=10)
vgm.gau= vgm(model="Gau",range=40000,nugget=10)
...
INC_r.vgm = variogram(log(INC_CNT+eps)~X_COORD+Y_COORD, cutoff = 1e5, data = sub.df)
vgm.sph= vgm(model="Sph",range=40000, nugget=10)
...
INC.dirvgm = variogram(log(INC_CNT+eps)~1, cutoff = 1e5, data = sub.df, alpha=c(0,45,90,135))
...
dirvgm.sph = vgm(model="Sph",range=40000,nugget=10,anix=c(0,0.6))
...
for (j in 1:12){
  ...
  INCm.vgm = variogram(log(INC_CNT+eps)~1,cutoff = 1e5,data=Msub.df)
  par(mar=c(4,4,3,1.5))
...
for (k in unique(CrimeData$YEAR)){
  ...
  INCy.vgm = variogram(log(INC_CNT+eps)~1,cutoff = 1e5,data=Ysub.df)
...
nobs <- 100 
ptm <- proc.time()
INC.kriged = krige(log(INC_CNT+eps)~1, locations = sub.df, newdata = interp.grd, nmax = nobs, model = INC.sphfit)
...
# plot(sqrt(INC.krigedDF$Var),pch=16,cex=0.75,col="blue",ylab="standard deviation")
```